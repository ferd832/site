    ...
    <div className="glass rounded-2xl p-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <button onClick={() => { if (audioRef.current) { audioRef.current.currentTime = Math.max(0, audioRef.current.currentTime - 5); segmentEndRef.current = null; } }} className="text-purple-300/70 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/5" title="Назад 5 сек">
          ...
          <button onClick={togglePlay} className={`w-12 h-12 rounded-full flex items-center justify-center transition-all cosmic-btn ${isSyncing ? 'animate-pulse' : ''}`}>
          ...
          <button onClick={() => { if (audioRef.current) { audioRef.current.currentTime = Math.min(duration, audioRef.current.currentTime + 5); segmentEndRef.current = null; } }} className="text-purple-300/70 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/5" title="Вперёд 5 сек">
